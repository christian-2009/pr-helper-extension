...
const comment_with_reply_mock_html = fs.readFileSync(
  path.resolve(
    __dirname,
    '../../tests/__fixtures__/comment_with_reply_mock.html',
  ),
  'utf-8',
);
const pr_with_multiple_files_and_comments = fs.readFileSync(
  path.resolve(
    __dirname,
    '../../tests/__fixtures__/pr_with_multiple_files_and_comments.html',
  ),
  'utf-8',
);
...
describe('CommentData', () => {
  ...
  describe('isLastReplyFromAssignee', () => {
    ...
    it.each([
      ...
      (lastCommentAuthor, expectedResult) => {
        // Given
        const commentElement = document.createElement('div');
  ...
  describe('getFileName', () => {
    it.each([
      ...
      (numberOfFile: number, expectedFileName: string) => {
        // Given
        ...
        const fileElement = container.querySelector(
          `.js-file.js-details-container:nth-child(${numberOfFile})`,
        );
